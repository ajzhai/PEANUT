...
def nearest_dist(pred, gt, k=1, voxel_size=0.05):
    """
    ...
    k_locs.append(pred_loc)
    xv, yv = np.meshgrid(range(pred.shape[0]), range(pred.shape[1]))
    
    while len(k_locs) < k:
        dd = np.sqrt((xv - pred_loc[0])**2 + 
                     (yv - pred_loc[1])**2)
        pred[dd < (1 / voxel_size)] -= 1
        pred_loc = np.argmax(pred)
        pred_loc = pred_loc // pred.shape[1], pred_loc % pred.shape[1]
        k_locs.append(pred_loc)
...
if __name__ == '__main__':
    ...
    for train_i in [48000]:
        ...
        if quan:
            ...
            dists_global = [[] for c in range(n_c)]
            nlls_global = [[] for c in range(n_c)] 
            bces_global = [[] for c in range(n_c)] 
            ...
            for i, n in enumerate(fnames):
                ...
                for t_idx in [0, 1, 3, 7]:
                    ...
                    result_global = inference_smp(model_global,  osp.join(data_dir, n), t_idx=t_idx)
                    ...
                    result_global[0] = result_global[0][:, center[0] - cropsize:center[0] + cropsize, 
                                                        center[1] - cropsize:center[1] + cropsize]
                    
                    # result = inference_smp_crop(model,  osp.join(data_dir, n), t_idx=t_idx, center=center)
                    # result_global = inference_smp_crop(model_global,  osp.join(data_dir, n), t_idx=t_idx, center=center)
                    ex_mask = 1 - (mf[t_idx, 1, center[0] - cropsize:center[0] + cropsize, 
                                                        center[1] - cropsize:center[1] + cropsize] > 0)
                    
                    for c in range(n_c):
                        # pred = result[0][c]
                        gt = obj_map[c][center[0] - cropsize:center[0] + cropsize, 
                                        center[1] - cropsize:center[1] + cropsize] * ex_mask
                        # dist = nearest_dist(pred, gt, k=3)
                        ...
                        pred = result_global[0][c]
                        dist = nearest_dist(pred, gt, k=5)
                        nll = neg_log_likelihood(pred, gt)
                        bce = bce_loss(pred, gt)
                        if dist >= 0:
                            dists_global[c].append(dist)
                            nlls_global[c].append(nll)
                        bces_global[c].append(bce)
            ...
            for c in range(n_c):
                print('%12s: %.3f MSE, %.3f NLL, %.5f BCE, %d occurrences' % 
                      (common_cls[c], np.mean(dists_global[c]), np.mean(nlls_global[c]), np.mean(bces_global[c]), len(dists[c])))
            all_dists = sum(dists_global, [])
            all_nlls = sum(nlls_global, [])
            all_bces = sum(bces_global, [])
            print('-' * 60)
            print('%12s: %.3f MSE, %.3f NLL, %.5f BCE, %d occurrences' % 
                  ('ALL CLASSES', np.mean(all_dists), np.mean(all_nlls), np.mean(all_bces), len(all_dists)))