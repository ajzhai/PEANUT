...
class LoadMapFromFile(object):
    ...
    def __call__(self, results):
        ...
        rand_ang = np.random.randint(0, 12) * 30
        img = ndimage.rotate(maps[results['img_info']['t_idx']], rand_ang, axes=(2, 1), order=1, reshape=False)
        
        img = img.astype(np.float32) / 255.
        locs = np.where(img[3, :, :] > 0)
        rand_idx = np.random.randint(len(locs[0]))
        rand_dx = np.random.randint(-5, 6)
        rand_dy = np.random.randint(-5, 6)
        center = (min(max(60, locs[0][rand_idx] + rand_dx), img.shape[1] - 60), 
                  min(max(60, locs[1][rand_idx] + rand_dy), img.shape[2] - 60))
        img = img[:, center[0] - 60:center[0] + 60, center[1] - 60:center[1] + 60]
        ...
        final_map = ndimage.rotate(maps[-1, goals], rand_ang, axes=(2, 1), order=1, reshape=False)
        final_map_crop = final_map[:, center[0] - 60:center[0] + 60, center[1] - 60:center[1] + 60]
        results['gt_semantic_seg'] = (final_map_crop * (1 - mask)).transpose(1, 2, 0)
...
def my_loss(pred, target):
    ...
    pos_weight = torch.ones(pred[0].shape).to(pred.device) #torch.ones(6) * 10
...
if __name__ == '__main__':
    ...
    cfg.data.train.img_dir = 'train_56' if use_rn else 'train_80' #'train_full' 
    ...
    cfg.work_dir =  '/shared/perception/personals/albert/work_dirs/ego_80_t10' #'../work_dirs/smp_weighted_t10'
